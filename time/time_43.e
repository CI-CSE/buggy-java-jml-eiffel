...
feature
	...
	get_time: TIME_43
		do
			Result := create {TIME_43}.make_2 (hour, minute, second)
		ensure
	...
	later_than (a_time: TIME_43): BOOLEAN
		do
	...
	equals (a_object: ANY): BOOLEAN
		...
		-- 	status: functional
		do
			Result := if attached {TIME_43} a_object as a_time then
				not (hour = a_time.get_hour and minute = a_time.get_minute and second = a_time.get_second) -- (hour = a_time.get_hour and minute = a_time.get_minute and second = a_time.get_second)
			else
				False
			end
		ensure
			a_object_not_time: not attached {TIME_43} a_object as a_time implies Result = False
			a_object_is_time: attached {TIME_43} a_object as a_time implies Result = (hour = a_time.get_hour and minute = a_time.get_minute and second = a_time.get_second)
		end
feature {NONE} -- Ghost variables

	stop_minute (a_start, a_stop: TIME_43): INTEGER
		note
	...
	stop_hour (a_start, a_stop: TIME_43): INTEGER
		note
			status: functional, ghost
			explicit: wrapping, contracts
		require
	...
	diff_hour (a_start, a_stop: TIME_43): INTEGER
		note
			status: functional, ghost
			explicit: wrapping, contracts
		require
	...
	diff_minute (a_start, a_stop: TIME_43): INTEGER
		note
			status: functional, ghost
			explicit: wrapping, contracts
		require
	...
	diff_second (a_start, a_stop: TIME_43): INTEGER
		note
			status: functional, ghost
			explicit: wrapping, contracts
		require
...
feature 
	trusted_difference (a_start, a_stop: TIME_43): TIME_43
		note
			explicit: wrapping, contracts
		require
			a_start_is_wrapped: a_start.is_wrapped
	...
	difference (a_start, a_stop: TIME_43): TIME_43
		note
			explicit: wrapping, contracts
		require
			a_start_is_wrapped: a_start.is_wrapped
		...
		do
			...
			a_stop_gt_a_start: a_stop.later_than (a_start) ⇒ (Result.get_hour = (trusted_difference (a_start, a_stop)).get_hour and Result.get_minute = (trusted_difference (a_start, a_stop)).get_minute and Result.get_second = (trusted_difference (a_start, a_stop)).get_second)
		end

	time_options (a_start, a_stop: TIME_43; a_sel: INTEGER): TIME_43
		note
			status: impure
			explicit: wrapping, contracts
		require
		...
		do
			...
			a_sel_3_a_start_neq_a_stop: ((a_sel = 3 ∧ (not (a_start.equals (old a_stop)).old_)) ⇒ (Result.get_hour = hour ∧ Result.get_minute = minute ∧ Result.get_second = second)) 
			a_sel_3_a_start_eq_a_stop: ((a_sel = 3 ∧ ((a_start.equals (old a_stop)).old_)) ⇒ (Result.get_hour = 0 ∧ Result.get_minute = 0 ∧ Result.get_second = 0 ∧ a_start.get_hour = 0 ∧ a_start.get_minute = 0 ∧ a_start.get_second = 0 ∧ a_stop = old a_stop)) 
			a_sel_4: a_sel = 4 ⇒ (Result.get_hour = difference (a_start, a_stop).get_hour ∧ Result.get_minute = difference (a_start, a_stop).get_minute ∧ Result.get_second = difference (a_start, a_stop).get_second ∧ a_start = old a_start ∧ a_stop = old a_stop)
		end
