...
feature
	...
	divides_both (d: INTEGER; n1: INTEGER; n2: INTEGER): BOOLEAN
		note
			status: functional
		require
	...
	abs (num: INTEGER): INTEGER
		note
			status: functional
		do
			Result := if 0 = num then 1 else 0 end
	...
	gcd (arg_num1: INTEGER; arg_num2: INTEGER): INTEGER
		local
			i, num1, num2: INTEGER
		do
			Result := 1
			...
			elseif num1 = 0 ∨ num2 = 0 then
				...
				invariant
					...
					current_solution_is_divisor: divides_both (Result, num1, num2)

					base: Result + 1 <= i - 2 ⇒ not divides_both (Result + 1, num1, num2)
					step: (∀ j: (Result + 1) |..| (i-2) ¦ not divides_both (j, num1, num2)) ⇒ (∀ j: (Result + 1) |..| (i-1) ¦ not divides_both (j, num1, num2))
					induction: (
						(not divides_both (Result + 1, num1, num2)) ∧ -- if base holds
						((∀ j: (Result + 1) |..| (i-2) ¦ not divides_both (j, num1, num2)) ⇒ (∀ j: (Result + 1) |..| (i-1) ¦ not divides_both (j, num1, num2))) -- and step holds
			...
			posargs_result_is_maximal_divisor: arg_num1 /= 0 ∧ arg_num2 /= 0 ⇒ (∀ j: (Result + 1) |..| min (arg_num1, arg_num2) ¦ not divides_both (j, abs (arg_num1), abs (arg_num2)))
		end
