...
feature
	...
	factorial (a_n: INTEGER): INTEGER
		...
		do
			...
			valid_result: Result = spec_factorial (a_n)
		end

	spec_factorial (a_n: INTEGER): INTEGER
		require
		...
		do
			if a_n = 0 then
				...
				Result := a_n * spec_factorial (a_n - 1)
			end
		ensure
			a_n_greater_than_zero: (a_n > 0) implies (Result = (a_n * spec_factorial (a_n - 1)))
