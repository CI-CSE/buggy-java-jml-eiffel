...
feature
	...
	enter (data: INTEGER)
		...
		do
			...
			not_full_rear: not (old is_full) ⇒ (old rear) = rear - 1
			not_full_set: not (old is_full) ⇒ queue.sequence [rear - 1] = data
		end
	...
	peek: INTEGER
		...
		do
			...
			if_not_empty: not is_empty ⇒ Result = queue.sequence [front]
		end
